...
bool **citire_mat_de_adiacenta(int *nr_noduri){
     bool **mat_de_adiacenta = new bool*[*nr_noduri];
     for(unsigned int i = 0; i < *nr_noduri; ++i)
          mat_de_adiacenta[i] = new bool[*nr_noduri]{ false };
...
std::vector<int> *det_noduri_izolate(int *n, bool **mat_de_adiacenta){
...
int **det_mat_distantelor(int *n, bool **mat_de_adicenta){
     ...
     for(unsigned int k = 0; k < *n; ++k)
          for(unsigned int i = 0; i < *n; ++i)
               for(unsigned int j = 0; j < *n; ++j)
                    ...
                    else if(i != j && mat_distantelor[i][j] > mat_distantelor[i][k] + mat_distantelor[k][j])
                         mat_distantelor[i][j] = mat_distantelor[i][k] + mat_distantelor[k][i];
...
int main(){
     int *n = new int{ 0 };
     fin >> *n;
     bool **mat_de_adiacenta = citire_mat_de_adiacenta(n);

     std::vector<int> *noduri_izolate = det_noduri_izolate(n, mat_de_adiacenta);
     ...
     int **mat_distantelor = det_mat_distantelor(n, mat_de_adiacenta);
     for(unsigned int i = 1; i < *n; ++i, std::cout<<'\n')
          for(unsigned int j = 0; j < *n; ++j)
